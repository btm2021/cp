...
const fs = require('fs');
async function pnSend(title, message) {
  return new Promise((resolve, reject) => {
    console.log("[PN].Send message ", message);
    pubnub.publish({
      channel: "crypto",
      message: {
        title: title,
...
pnSend("APP", `[WSListen] - Module WSListen bắt đầu hoạt động`)
var ListAccount = [];
let accountJSONFile = fs.readFileSync("./accounts.json")
ListAccount = JSON.parse(accountJSONFile)
var accMaster;
var wsMaster;
var accSlave = [];
function main() {
  ...
  ListAccount.map(acc => {
    if (acc.role === "slave" && acc.status === 'true') {
      pnSend("APP", `Đăng kí acc slave ${acc.name}`)
      accSlave.push(
        {
          binance:
            new Binance().options({
    ...
    if (acc.role === "master" && acc.status === 'true') {
      pnSend("APP", `Đăng kí acc Master ${acc.name}`)
      accMaster = acc;
...
function initWebSocket() {
  if (wsMaster) {
    wsMaster.on('open', (data) => {
      pnSend("APP", `Bắt đầu lắng nghe trên acc master ${accMaster.name}`)
...
function changeLeverage(symbol, leverage) {
  ...
  pnSend("Cấu hình", `[Đòn bẩy]-Master thay đổi đòn bẩy trên [${symbol}x${leverage}]`);
  let listPromise = [];
  accSlave.map(acc => {
...
async function oneChangeLeverage(symbol, leverage, acc) {
  return new Promise((resolve, reject) => {
    acc.binance.futuresLeverage(symbol, leverage).then(data => {
      pnSend("Cấu hình", `[Đòn bẩy]- Thay đổi đòn bẩy trên acc ${acc.name} + ${symbol} + X${leverage}  *SUCCESS`).then(data => {
        resolve('ok')
      })
    }).catch(err => {
      pnSend("Cấu hình", `[Đòn bẩy]- Thay đổi đòn bẩy trên acc ${acc.name} + ${symbol} + X${leverage}  *FAIL`).then(data => {
        reject(err)
      })
    })
  })
...
function makeTPOrder(data) {
  ...
  let listPromise = [];
  console.log("Master Tạo TP Order " + symbol + ":" + price)
...
function makeSLOrder(data) {
  ...
  let listPromise = [];
  console.log("Master Tạo SL Order " + symbol + ":" + price)
